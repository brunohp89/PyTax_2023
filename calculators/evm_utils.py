...
def get_transactions_raw(address, chain, scan_key=None, return_full_names=False):
    ...
    trx_df["timeStamp_normal"] = [
        utils.date_from_timestamp(k) if not isinstance(k, float) else None
        for k in trx_df["timeStamp_normal"]
    ]
    trx_df["timeStamp_internal"] = [
        utils.date_from_timestamp(k) if not isinstance(k, float) else None
        for k in trx_df["timeStamp_internal"]
    ]
    trx_df["timeStamp"] = [
        utils.date_from_timestamp(k) if not isinstance(k, float) else None
        for k in trx_df["timeStamp"]
    ]
    trx_df["timeStamp_erc721"] = [
        utils.date_from_timestamp(k) if not isinstance(k, float) else None
        for k in trx_df["timeStamp_erc721"]
    ]
    trx_df["timeStamp_erc1155"] = [
        utils.date_from_timestamp(k) if not isinstance(k, float) else None
        for k in trx_df["timeStamp_erc1155"]
    ]
...
def weth(weth_df, gas_coin, columns_keep):
    if weth_df.shape[0] > 0:
        ...
        weth_df[["value_normal", "value_internal"]] = weth_df[
            ["value_normal", "value_internal"]
        ].map(lambda c: 0 if pd.isna(c) else int(c))
        ...
        weth_df.loc[
            weth_df["functionName"].str.contains("withdraw"), "value_normal"
        ] *= -1
